...
class DecaUSBCrg(LiteXModule):
    ...
    def __init__(self, platform, sys_clk_freq, ulpi=None, clk60=None, sys_from_usb=True, with_por=True):
        ...
        if clk60 is not None and ulpi is not None:
            ...
            # Free-run: never reset the USB PLL. A reset here would be
            # launched from the sys domain (LiteX drives crg.rst from the
            # soc_rst CSR) and timed against clk600 — an artificial
            # cross-domain path. The PLL must keep running to keep REFCLK
            # up; PHY/POR sequencing is handled by the por counter below.
...
# SDC (ULPI I/O timing + CDC false paths) --------------------------------------

def deca_usb_sdc(platform):
    """ULPI I/O timing + clock-domain-crossing constraints for DECA designs.

    Needed by EVERY SoC that touches the ULPI bus on the DECA (device AND
    host examples), regardless of whether it derives from DecaUSBSoC.
    """
    ...
    #
    # Set LITEUSB_NO_ULPI_TIMING=1 to skip the I/O constraints (LUNA-style
    # "just let the fitter handle it" approach).
    if not int(os.getenv("LITEUSB_NO_ULPI_TIMING", "0")):
        platform.toolchain.additional_sdc_commands += [
            ...
            # FPGA->PHY outputs: usb_clk is the PLL output with a -120°
            # phase shift, so data launched on usb_clk targets the PHY's
            # SECOND clk600 sampling edge (the first arrives too early
            # for the datasheet 6ns setup).
            "set_multicycle_path 2 -setup -from [get_clocks {usb_clk}] -to [get_clocks {clk600}]",
            "set_multicycle_path 1 -hold  -from [get_clocks {usb_clk}] -to [get_clocks {clk600}]",
            # ULPI bus turnaround: DIR -> DATA is a pin-to-pin release path
            # of the tristate driver (the controller inserts whole-cycle
            # bus turnarounds; the PHY never drives during that window).
            "set_false_path -from [get_ports {ulpi0_dir}] -to [get_ports {ulpi0_data[*]}]",
        ]
    # The POR counter runs on the raw clk50 pin; its CDC into the usb
    # domain is quasi-static (counts down once, then constant).
    # The PLL output clock names are generated by derive_pll_clocks
    # (e.g. 'altpll:ALTPLL|...|clk[0]'); match all of them with a
    # glob regardless of PLL count or hierarchy depth.
    platform.toolchain.additional_sdc_commands += [
        "set_false_path -from [get_clocks {clk50}] -to [get_clocks {usb_clk}]",
        "set_false_path -from [get_clocks {clk50}] -to [get_clocks {*altpll*}]",
    ]
...
class DecaUSBSoC(SoCCore):
    ...
    def __init__(self, sys_clk_freq=50e6,
        ...
        **kwargs):
        ...
        # ULPI I/O timing + CDC constraints (shared with the host examples).
...
def deca_main(soc_cls, description, output_dir=None):
    ...
    if output_dir is not None:
        # Keep each example's build out of the default platform-named dir
        # (e.g. deca_ehci_host.py -> build/deca_ehci_host).
