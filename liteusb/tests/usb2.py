...
class USBDeviceTest:
    ...
    def instantiate_dut(self):
        ...
        arguments = getattr(self, 'FRAGMENT_ARGUMENTS', {}).copy()
    ...
    def send_token(self, pid, *, endpoint=0, address=None):
        ...
        # Grab the raw bits that make up our token from the usb_packet library...
    ...
    def send_handshake(self, pid):
        ...
        pid = int(pid)
        yield from self.provide_packet(USBPacketID.byte(pid))
    ...
    def receive_packet(self, as_bytes=True, timeout=1000):
        ...
        timeout_counter = 0
        while not (yield self.utmi.tx_valid):
            timeout_counter += 1
            if timeout_counter >= timeout:
                raise TimeoutError("Timeout waiting for TX valid")
            yield
    ...
    def wait_until(self, signal, timeout=1000):
        """ Wait until a signal becomes true. """
        timeout_counter = 0
    ...
    def out_transaction(self, *octets, endpoint=0, token_pid=USBPacketID.OUT,
        ...
        Parameters:
            *octets          -- The data to send.
            ...
            data_pid         -- The data PID to send. Defaults to DATA0.
    ...
    def out_transfer(self, *octets, endpoint=0, data_pid=USBPacketID.DATA0, max_packet_size=64):
        """ Performs an OUT transfer (multiple transactions if needed).

        Parameters:
            *octets          -- The data to send.
            endpoint         -- The endpoint on which to send the relevant data.
            data_pid         -- The first data PID to send. Defaults to DATA0.
    ...
    def in_transaction(self, endpoint=0, data_pid=None, handshake=USBPacketID.ACK):
        ...
        Parameters:
            ...
            data_pid    -- The data PID to expect, or None if we don't care.
            handshake   -- The response we should give after receiving data.

        Returns:
            pid    -- The PID of the received packet.
        ...
        if data_pid is not None:
            self.assertEqual(pid, USBPacketID.byte(data_pid))
    ...
    def in_transfer(self, endpoint=0, data_pid=None, handshake=USBPacketID.ACK):
        """ Performs an IN transfer (multiple transactions if needed).
        ...
        Returns:
            pid    -- The PID of the last data packet received.
            data   -- A list of octets received.
        """
        ...
        while True:
            pid, packet = yield from self.in_transaction(
                endpoint=endpoint, data_pid=data_pid, handshake=USBPacketID.ACK)
            ...
            if packet is None or len(packet) < self.max_packet_size_ep0:
                break
    ...
    def control_interphase_delay(self):
        """ Waits for a period appropriate between control transfer phases. """
    ...
    def get_descriptor(self, descriptor_type, index=0, length=64):
        ...
        Returns:
            handshake -- The handshake received.
            data      -- The descriptor data.
        """
    ...
    def set_address(self, new_address, update_address=True):
        ...
        Parameters:
            ...
            update_address  -- If True, update self.address on success.

        Returns:
            response_pid -- The handshake PID received.
        """
    ...
    def get_configuration(self):
        ...
        Returns:
            response -- Tuple of (handshake, data).
        """
