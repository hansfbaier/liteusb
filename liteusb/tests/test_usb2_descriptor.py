...
class GetDescriptorHandlerBlockTest(LiteUSBUSBTestCase):
    ...
    def _test_descriptor(self, type_number, index, raw_descriptor, start_position, max_length, delay_ready=0):
        ...
        # Set up request
        yield self.dut.value[8:16].eq(type_number)  # Type
        yield self.dut.value[0:8].eq(index)  # Index
    ...
    def test_unavailable_descriptor(self):
        yield from self._test_stall(DescriptorTypes.STRING, 100, 0, 64)
