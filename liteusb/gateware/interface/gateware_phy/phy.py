...
from migen import Signal, Module, Cat, ClockSignal

from .receiver      import RxPipeline
from .transmitter   import TxPipeline


class GatewarePHY(Module):
    ...
    def __init__(self, *, io):
        ...
        #
        counter = Signal(2)
        in_normal_mode       = Signal()
        in_non_encoding_mode = Signal()
        ...
        self.comb += self.line_state.eq(Cat(self._io.d_n.i, self._io.d_p.i))
        ...
        if hasattr(self._io, 'pullup'):
            self.comb += self._io.pullup.o.eq(self.term_select)

        # If we have a pulldown signal, drive it based on our pulldown controls.
        if hasattr(self._io, 'pulldown'):
            self.comb += self._io.pullup.o.eq(self.dm_pulldown | self.dp_pulldown)
        ...
        #
        self.comb += [
            in_normal_mode.eq(self.op_mode == self.OP_MODE_NORMAL),
            in_non_encoding_mode.eq(self.op_mode == self.OP_MODE_NO_ENCODING),
        ]

        transmitter = TxPipeline()
        ...
        # Override outputs when in non-encoding mode
        self.comb += [
            If(in_non_encoding_mode,
                # USB output.
        ...
        self.sync.usb_io += counter.eq(counter + 1)
        self.comb += transmitter.i_bit_strobe.eq(counter == 0)

        #
        # Receiver
        #
        receiver = RxPipeline()
        self.submodules.receiver = receiver
        self.comb += [
        ...
        self.sync.usb += self.rx_complete.eq(receiver.o_pkt_end)
