...
class TransactionalizedFIFO(Module):
    ...
    def __init__(self, *, width, depth, name=None, domain="sync"):
        ...
        self.space_available  = Signal(max=depth + 1)
        ...
        self.comb += [
            ...
            write_port.we.eq(self.write_en & ~self.full)
        ]
        ...
        sync_domain += [
            If(self.write_en & ~self.full,
                current_write_pointer.eq(next_write_pointer)
            ),
            If(self.write_commit,
                committed_write_pointer.eq(current_write_pointer)
            ),
            If(self.write_discard,
                current_write_pointer.eq(committed_write_pointer)
            )
        ]

        self.comb += [
            If(current_write_pointer == depth,
                next_write_pointer.eq(0)
            ).Else(
                next_write_pointer.eq(current_write_pointer + 1)
            )
        ]
        ...
        self.comb += [
            If(current_read_pointer == depth,
                next_read_pointer.eq(0)
            ).Else(
                next_read_pointer.eq(current_read_pointer + 1)
            )
        ]
        ...
        self.comb += [
            If(self.read_en & ~self.empty,
                read_port.adr.eq(next_read_pointer)
            ).Else(
                read_port.adr.eq(current_read_pointer)
            )
        ]
        ...
        sync_domain += [
            If(self.read_en & ~self.empty,
                current_read_pointer.eq(next_read_pointer)
            ),
            If(self.read_commit,
                committed_read_pointer.eq(current_read_pointer)
            ),
            If(self.read_discard,
                current_read_pointer.eq(committed_read_pointer)
            )
        ]
        ...
        self.comb += self.empty.eq(current_read_pointer == committed_write_pointer)
        ...
        self.comb += [
            If(self.full,
                self.space_available.eq(0)
            ).Elif(committed_read_pointer <= current_write_pointer,
                self.space_available.eq(depth - (current_write_pointer - committed_read_pointer))
            ).Else(
                self.space_available.eq(committed_read_pointer - current_write_pointer - 1)
            )
        ]

        # Our FIFO is full if we don't have any space available.
        self.comb += self.full.eq(next_write_pointer == committed_read_pointer)
