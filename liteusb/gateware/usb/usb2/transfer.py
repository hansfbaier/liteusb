...
class USBInTransferManager(Module):
    ...
    flush: Signal(), input
        ...
        not wait further for the input stream to end, or to have a max packet size; it will send a
    ...
    def __init__(self, max_packet_size):
        ...
        buffer_fill_count   = Array(Signal(max=self._max_packet_size + 1, name=f"fill_count_{i}") for i in range(2))
        ...
        self.sync.usb += If(buffer_write.we,
        ...
        # Synchronous updates performed in WAIT_FOR_DATA when a packet becomes ready.
        ...
        fsm.act("WAIT_TO_SEND",
            ...
            # If we get a clear halt request while in this state, reset to initial PID.
                ...
                NextState("WAIT_TO_SEND"),
                NextValue(self.data_pid, self.start_with_data1)
        ...
        # If we send a ZLP, clear the need to follow up with one.
        self.sync.usb += If(fsm.ongoing("WAIT_TO_SEND") & in_token_received & (read_fill_count == 0),
            read_stream_ended.eq(0)
        )
        ...
        fsm.act("SEND_PACKET",
            out_stream.valid.eq(1),
            out_stream.last.eq(send_position + 1 == read_fill_count),
            ...
            If(out_stream.ready,
                ...
                If(send_position + 1 == read_fill_count,
                    NextState("WAIT_FOR_ACK")
                )
            )
        )
        ...
        # Synchronous updates performed in WAIT_FOR_ACK when the host ACKs.
