...
class USBControlEndpoint(Module):
    ...
    def do_finalize(self):
        # This method replaces elaborate() in migen
        ...
        if self._standalone:
            ...
            # Create our timer...
            timer = USBInterpacketTimer()
            self.submodules.timer = timer
            timer.add_interface(interface.timer)

            # ... our CRC generator ...
            crc = USBDataPacketCRC()
            ...
            # ... and our tokenizer.
            tokenizer = USBTokenDetector(utmi=self.utmi)
            self.submodules.token_detector = tokenizer
            self.comb += tokenizer.interface.connect(interface.tokenizer)
        ...
        setup_decoder = USBSetupDecoder(utmi=self.utmi)
        self.submodules.setup_decoder = setup_decoder
        self.comb += [
        ...
        request_mux = USBRequestHandlerMultiplexer()
        self.submodules.request_mux = request_mux
        ...
        for handler in self._request_handlers:
            ...
            # ... and add it.
            setattr(self.submodules, name, handler)
        ...
        fsm.act("SETUP",
            ...
            If(setup_decoder.packet.received & endpoint_targeted,
                ...
                If(setup_decoder.packet.length,
                    # If this is an device -> host request, expect an IN packet.
                    If(setup_decoder.packet.is_in_request,
                        NextState("DATA_IN")
        ...
        fsm.act("DATA_IN",
            ...
            If(interface.tokenizer.new_token & interface.tokenizer.is_setup,
                NextState("SETUP")
            # Wait until we have an IN token, and are allowed to respond to it.
            ).Elif((interface.tokenizer.ready_for_response & endpoint_targeted) & interface.tokenizer.is_in,
                # Notify the request handler to prepare a response.
                request_handler.data_requested.eq(1)
            ),
            # Once we get an OUT token, we should move on to the STATUS stage. [USB2, 8.5.3]
            If(endpoint_targeted & interface.tokenizer.new_token & (interface.tokenizer.is_out | interface.tokenizer.is_ping),
                NextState("STATUS_OUT")
            )
        )
        ...
        fsm.act("DATA_OUT",
            ...
            If(endpoint_targeted & interface.tokenizer.is_out,
            ...
            If(endpoint_targeted & interface.tokenizer.new_token & interface.tokenizer.is_in,
                NextState("STATUS_IN")
            ),
            # Respond to PING token [USB2.0: 8.5.1]
            If(endpoint_targeted & interface.tokenizer.ready_for_response & interface.tokenizer.is_ping,
        ...
        fsm.act("STATUS_IN",
            ...
            If((interface.tokenizer.ready_for_response & endpoint_targeted) & interface.tokenizer.is_in,
                request_handler.status_requested.eq(1)
            )
        )
        ...
        fsm.act("STATUS_OUT",
            ...
            If((interface.rx_ready_for_response & endpoint_targeted) & interface.tokenizer.is_out,
                request_handler.status_requested.eq(1)
            ),
            # Respond to PING token [USB2.0: 8.5.1]
            If(endpoint_targeted & interface.tokenizer.ready_for_response & interface.tokenizer.is_ping,
