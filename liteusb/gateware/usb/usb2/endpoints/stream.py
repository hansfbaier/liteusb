...
class USBStreamInEndpoint(Module):
    ...
    def __init__(self, *, endpoint_number, max_packet_size):
        ...
        self.submodules.tx_manager = tx_manager = USBInTransferManager(self._max_packet_size)
...
class USBMultibyteStreamInEndpoint(Module):
    ...
    def __init__(self, *, byte_width, endpoint_number, max_packet_size):
        ...
        self.submodules.stream_ep = stream_ep = USBStreamInEndpoint(
        ...
        bytes_to_send = Signal(max=self._byte_width + 1)
...
class USBStreamOutEndpoint(Module):
    ...
    def __init__(self, *, endpoint_number, max_packet_size, buffer_size=None):
        ...
        # Create a Rx FIFO.
        self.submodules.fifo = fifo = TransactionalizedFIFO(width=10, depth=self._buffer_size, name="rx_fifo", domain="usb")
        ...
        self.sync.usb += [
            If(fifo.write_en,
                ...
                If(rx_last,
                    transfer_active.eq(full_packet)
                )
            ),
            ...
            If(fifo.write_commit | fifo.write_discard,
                overflow.eq(0),
                rx_cnt.eq(0)
            ),

            # We'll toggle our DATA PID each time we issue an ACK to the host [USB 2.0: 8.6.2].
            If(data_response_requested & data_accepted,
                expected_data_toggle.eq(~expected_data_toggle)
            )
        ]
